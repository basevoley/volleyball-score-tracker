...
const PlayerRow = ({ player, index, onUpdate, onDelete }) => {
    const [localValue, setLocalValue] = useState({ name: player.name, number: player.number });
    ...
    const handleBlur = (field) => {
        if (localValue[field] !== player[field]) {
            onUpdate(index, field, localValue[field]);
        }
    };
...
const TeamPlayerList = ({ teamId, matchDetails, setMatchDetails }) => {
