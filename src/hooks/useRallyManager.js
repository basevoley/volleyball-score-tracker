...
export const useRallyManager = (initialServer, onPossessionChange) => {
    ...
    const initialPossession = useRef(initialServer || null);
    const previousPossession = useRef(initialServer || null);
    ...
    // FAULT ACTION
    const handleFault = useCallback((faultingTeam) => {
        setRally(prev => {
    ...
    const handleAction = useCallback((action, faultingTeam = null) => {
        switch (action) {
            case 'serve':
                handleServe();
                break;
    ...
    const updateInitialServer = useCallback((newServer) => { initialPossession.current = newServer; previousPossession.current = newServer; setRally(prev => ({ ...prev, possession: newServer, })); }, []);

    // TOGGLE CONFIRMATIONS 
    const setConfirmation = useCallback((show) => { setRally(prev => ({ ...prev, showConfirmation: show })); }, []);

    const setDiscardConfirmation = useCallback((show) => { setRally(prev => ({ ...prev, showDiscardConfirmation: show })); }, []);
